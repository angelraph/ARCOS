 ...
 *
 * Adapted for ARCOS from Circle's arc-escrow sample (`RefundProtocol.sol`,
 * https://github.com/circlefin/arc-escrow), renamed to `Escrow` with no functional
 * changes. Kept deliberately decoupled from DecisionLedger: the ARCOS backend watches
 * this contract's events (PaymentCreated / Withdrawal / Refund) and separately calls
 * DecisionLedger.recordDecision(...) keyed by this contract's tx hash as `txRef`.
 *
 * Usage in the ARCOS flow:
 *  - `pay(to, amount, refundTo)`      -> Procurement Agent opens escrow for a supplier order (ESCROW_OPEN)
 *  - `withdraw(paymentIDs)`           -> Supplier Agent releases funds once delivery is AI-vision-confirmed (ESCROW_RELEASE)
 *  - `refundByArbiter(paymentID)`     -> Governance/arbiter wallet refunds the payer if delivery validation fails
 */
